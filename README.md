# Hola, soy Gian Carlos 👋

**Java Backend Developer | Spring Boot | Microservicios | Cloud**  
Lima, Perú 🇵🇪 | [gianpc.com](https://gianpc.com) | [LinkedIn](https://linkedin.com/in/gian-pc)

---

## 🚀 Sobre mí

Java Backend Developer con más de 5 años de experiencia construyendo APIs REST y microservicios con Java y Spring Boot, aplicando TDD, principios SOLID y arquitectura limpia.

Trayectoria en sectores de minería, meteorología e infraestructura gestionando datos críticos. Tech Lead y finalista del **Hackathon ChurnInsight — Oracle ONE + Alura (2025)**, liderando un equipo de 8 personas en el diseño e implementación de una plataforma full-stack de predicción de churn.

---

## 🔧 Stack tecnológico

**Backend**  
![Java](https://img.shields.io/badge/Java_17-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=spring-boot&logoColor=white)
![Spring Cloud](https://img.shields.io/badge/Spring_Cloud-6DB33F?style=flat&logo=spring&logoColor=white)
![Quarkus](https://img.shields.io/badge/Quarkus-4695EB?style=flat&logo=quarkus&logoColor=white)

**Bases de datos**  
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Oracle DB](https://img.shields.io/badge/Oracle_DB-F80000?style=flat&logo=oracle&logoColor=white)

**Cloud & DevOps**  
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazon-aws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat&logo=microsoft-azure&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Maven](https://img.shields.io/badge/Maven-C71A36?style=flat&logo=apache-maven&logoColor=white)

**Calidad & Testing**  
![JUnit](https://img.shields.io/badge/JUnit_5-25A162?style=flat&logo=junit5&logoColor=white)
![Mockito](https://img.shields.io/badge/Mockito-78A641?style=flat&logo=java&logoColor=white)

---

## 🏆 Proyectos destacados

### 🔥 [RetainAI — Hackathon ChurnInsight Finalista](https://github.com/gian-pc/RetainAI)
**Oracle ONE + Alura 2025 · Tech Lead de equipo de 8 personas**

Plataforma full-stack de predicción de churn con arquitectura de microservicios.
- Backend: **Java 17 + Spring Boot** con APIs REST y Spring Security/OAuth2
- Microservicio ML: **Python + FastAPI** con modelo XGBoost (dataset de 9,701 registros)
- Frontend: Next.js · Persistencia: PostgreSQL · Contenedores: Docker
- Asistente conversacional integrado con **Google Gemini API**

`Java 17` `Spring Boot` `Spring Security` `PostgreSQL` `Docker` `Python` `FastAPI` `XGBoost` `OCI`

---

### ⚙️ [OpsBackend — API REST con Spring Cloud](https://github.com/gian-pc/opsbackend-java)
**Proyecto de portafolio · Java Backend**

API REST de gestión de operaciones empresariales con arquitectura de microservicios.
- Java 17 + Spring Boot + Spring Cloud
- Autenticación JWT · Migraciones con Flyway · Documentación con Swagger
- Contenedores Docker · Base de datos PostgreSQL · Build con Maven

`Java 17` `Spring Boot` `Spring Cloud` `PostgreSQL` `Docker` `Flyway` `JWT` `Maven`

---

## 📜 Certificaciones

| Certificación | Institución | Año |
|---|---|---|
| OCI Foundations Associate | Oracle | 2025 |
| Spring Boot y Spring Cloud | JOEDAYZ | 2024 |
| Desarrollador Java Full Stack | JOEDAYZ | 2024 |
| Programación Java 17+ | JOEDAYZ | 2024 |
| Java Backend Developer | ATL Academy | 2023 |
| Sistemas Operativos y Linux | Coursera | 2023 |

---

## 📚 Formación

- 🎓 **Ingeniería de Sistemas** — Universidad Peruana de Ciencias Aplicadas (UPC) · 2023 – Actualidad
- 🏅 **Certified Tech Developer** — Digital House (Beca Globant / Mercado Libre) · 1,568 h · 2025
- 📊 **Oracle ONE — Especialización en Data Science** — Alura · 307 h · 2025

---

## 📬 Contacto

[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=flat&logo=gmail&logoColor=white)](mailto:gpaucarcortez@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/gian-pc)
[![Web](https://img.shields.io/badge/Web-gianpc.com-black?style=flat&logo=google-chrome&logoColor=white)](https://gianpc.com)
